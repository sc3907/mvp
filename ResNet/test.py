...
def plot_roc(df, y_true):
    ...
    for idx, algo in enumerate(algos):
        plt.plot(fpr[algo], tpr[algo], lw=lw,
                 label='{} ROC curve (area = {:.2f})'.format(
                     algo, roc_auc[algo]),
...
class CNN_Model(object):
    ...
    def __init__(self, input_shape=(20, 1, 1), weights_path=None, name='cnn_model',
        ...
        self.exclude_cols = {'target', 'CADD_phred', 'xEigen-phred', 'Eigen-PC-phred',
                             ...
                             'RVIS', 'obs_exp', 'mis_badness', 'MPC', 'REVEL'}
    ...
    def _init_model(self, verbose):
        ...
        if verbose:
            ...
            outname = '_'.join(
                ['../models/' + self.name, str(self.input_shape[0]), self.run_id,  'cols.png'])
            plot_model(self.model, show_shapes=True, to_file=outname)
            col_name = '_'.join(
                ['../models/' + self.name, str(self.input_shape[0]), self.run_id,  'col_names.txt'])
    ...
    def train(self, sub_sample):
        ...
        best_weights_filepath = '../models/' + self.name + '-' + str(n_cols) + 'cols' + \
...
class CNN_Model_Mode6(CNN_Model):
    ...
    def _init_model(self, verbose):
        ...
        if verbose:
            ...
            outname = '../models/' + self.name + '_' + \
                str(self.input_shape[0]) + 'cols' + '_' + self.run_id + '.png'
            plot_model(self.model, show_shapes=True, to_file=outname)
            col_name = '../models/' + self.name + '_' + \
                str(self.input_shape[0]) + 'cols' + '_' + self.run_id + '.txt'
            self._save_cols(col_name)
