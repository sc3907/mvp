...
def plot_roc(df, y_true):
    ...
    tpr = dict()
    roc_auc = dict()
    algos = ['MetaSVM_rankscore', 'MetaLR_rankscore', 'M-CAP_rankscore', 
    ...
    for idx, algo in enumerate(algos):
        plt.plot(fpr[algo], tpr[algo], lw=lw, 
                 label='{} (area = {:.3f})'.format(algo.split('_'), roc_auc[algo]),
...
class CNN_Model(object):
    ...
    print  "this is a version 46 features in total, no RVIS no mis_badness !"
    ...
    def __init__(self, input_shape=(20, 1, 1), weights_path=None, name='cnn_model',
        ...
        # ones with x infront will be used
        self.exclude_cols = {'var_id', 'target', 'CADD_phred', 
                             'xEigen-phred', 'xEigen-PC-phred',
                             'xEigen-PC-raw_rankscore', 'MetaSVM_rankscore',
    ...
    def _load_data(self, sub_sample=False):
        ...
        with open('../models/res_model_colname_'+datetime.datetime.now().strftime('%Y%m%d-%H.%M.%S')+'.txt', 'w') as fw:
            fw.write('{} cols used: {}'.format(len(cols), cols))
            fw.close()
    ...
    def train(self, sub_sample):
        ...
        best_weights_filepath = '../models/' + self.name + '-' + str(n_cols) + 'cols_' + datetime.datetime.now().strftime('%Y%m%d-%H.%M.%S') + '-weights-improvement-{epoch:02d}-{val_acc:.2f}.hdf5'
...
class CNN_Model_Mode6(CNN_Model):
    ...
    def _init_model(self, verbose):
        ...
        input_ = Input(shape=self.input_shape, name='input')
        x = Conv2D(nb_filters, kernel_size, padding="same", activation=activation)(input_)
        for _ in range(2):
            y = Conv2D(nb_filters, kernel_size, padding="same", activation=activation)(x)
